...
export const dataApi = {
  ...
  // UPDATED: Added scraped_by to params
  getAll: (params?: { status?: string; batch?: string; scraped_by?: string; limit?: number; offset?: number }) => {
    const searchParams = new URLSearchParams();
    if (params?.status && params.status !== 'all') searchParams.set('status', params.status);
    if (params?.batch && params.batch !== 'all') searchParams.set('batch', params.batch);
    if (params?.scraped_by && params.scraped_by !== 'all') searchParams.set('scraped_by', params.scraped_by);
...
export interface DataRow {
  ...
  profile_url: string | null; // ADDED THIS
...
export interface Stats {
  overall: { total: number; pending: number; completed: number; errors: number; };
  today: { total: number; completed: number; errors: number; };
  performance: Array<{ username: string; date: string; completed: number; errors: number; }>;
  users: Array<{ username: string; }>;
  byDate: Array<{ date: string; total: number; completed: number; errors: number; }>;
  batches: Array<{ batch_code: string; total_rows: number; created_at: string; scraped_at: string; completed: number; errors: number; }>;
}
